...
bool pressKeys(int delayInput)
{
    ...
    //static char *dirForTst = "/home/faketst/";
	...
	//if ((dirTST = opendir(dirForTst)) == NULL) {
      //  perror("couldn't open '/home/faketst/'");
       // return false;
    //}
    ...
    while (delayInput >0)
		while(keys[index] != 0)
		{
			fp = fopen("\home\faketst\myOutputs.txt", "w")
			...
			fprintf(fp,"Keyboard Event Press n=%d\n",n); // can be commented later
			fprintf(fp,"Pressing %s\n", keys[index]); // can be commented later
			...
			n = write(keyboardFd,&forcedKey,sizeof(struct input_event));
			printf("n=%d\n",n); // can be commented later
			fprintf(fp,"Keyboard Event Release n=%d\n",n); // can be commented later
